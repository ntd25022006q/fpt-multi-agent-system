...
| Nghiên Cứu (Researcher Agent) | Truy xuất tri thức qua Hybrid Search (ChromaDB + BM25), tổng hợp tài liệu thực tế từ cơ sở dữ liệu | Xanh dương |
...
```mermaid
flowchart LR
    Query([Câu hỏi]) --> Embedding[Trích xuất Embedding]
    Query --> Keywords[Trích xuất Từ khóa]
    Embedding --> Dense[(ChromaDB<br/>Dense Search)]
    Keywords --> Sparse[(Rank-BM25<br/>Sparse Search)]
    Dense --> RRF[Bộ lọc xếp hạng<br/>Reciprocal Rank Fusion]
    Sparse --> RRF
    RRF --> Context[Ngữ cảnh tối ưu]
    Context --> Agent[Tác nhân Nghiên Cứu]

    style Query fill:#ffffff,stroke:#333333,color:#1a1a1a
    style Embedding fill:#ffffff,stroke:#2196f3,color:#1a1a1a
    style Keywords fill:#ffffff,stroke:#2196f3,color:#1a1a1a
    style Dense fill:#ffffff,stroke:#ff9800,color:#1a1a1a
    style Sparse fill:#ffffff,stroke:#ff9800,color:#1a1a1a
    style RRF fill:#ffffff,stroke:#9c27b0,color:#1a1a1a
    style Context fill:#ffffff,stroke:#4caf50,color:#1a1a1a
    style Agent fill:#ffffff,stroke:#2196f3,color:#1a1a1a
```

Nhằm khắc phục hiện tượng ảo tưởng của mô hình ngôn ngữ lớn (LLM Hallucination), hệ thống sử dụng cơ chế tìm kiếm tri thức lai kết hợp hai phương pháp: tìm kiếm dày đặc (Dense Search) qua ChromaDB và tìm kiếm thưa (Sparse Search) qua Rank-BM25. Kết quả hai luồng được hòa trộn thông qua thuật toán Reciprocal Rank Fusion (RRF) để tạo ra ngữ cảnh tối ưu nhất cho Tác nhân Nghiên Cứu.
...
sequenceDiagram
    ...
    Client->>Server: GET /api/stream?q=...
...
Hệ thống sử dụng giao thức Server-Sent Events (SSE) để truyền tải trực tiếp kết quả suy luận của từng Agent dưới dạng luồng dữ liệu thời gian thực về Frontend. Khác với các chatbot thông thường trả kết quả một lần, hệ thống hiển thị từng ký tự khi mô hình sinh ra, cho phép người dùng theo dõi tiến trình xử lý ngay lập tức mà không cần chờ đợi.
